...
export const fetchEmployee = createAsyncThunk(
  ...
  async () => {
    const response = await axios.get("http://localhost:3004/api/items");
...
export const addEmployee = createAsyncThunk(
  ...
  async (data) => {
    const response = await axios.post("http://localhost:3004/api/items", {
      name: data.name,
...
export const removeEmployee = createAsyncThunk(
  ...
  async (data) => {
    const response = await axios.delete(
      `http://localhost:3004/api/items/${data}`
    );
    return response.data.response;
  }
);
...
export const modifiedEmployee = createAsyncThunk(
  ...
  async (data) => {
    const response = await axios.put(
      `http://localhost:3004/api/items/${data.id}`,
      {
        name: data.name,
        position: data.position,
      }
